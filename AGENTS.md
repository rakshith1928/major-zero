...
- **Seed is deterministic** (`seed/buses.py`): 82 bus templates, eight Bangalore-centred corridors — Chennai + Hyderabad (Vellore/Kurnool midpoints), Coimbatore (Salem) + Vijayawada (Anantapur), Goa (Hubballi) + Tirupati (Chittoor), Pondicherry (Vellore), Mysuru direct. Fares must stay in the Rs.600–1400 band (`test_seed_buses.py` enforces it). Stats shown in UI must come from seed/code, never invented.
